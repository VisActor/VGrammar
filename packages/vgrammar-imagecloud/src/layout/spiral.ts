...
import { field, setSize, visualizeSegmentation } from '../util';
...
export class SpiralLayout extends Layout {
  ...
  preProcess() {
    ...
    const { imageConfig = {}, size, ratio = 0.45 } = this.options;
  ...
  doLayout(images: ImageCollageInputType[]) {
    ...
    const { size, layoutConfig = {} as SpiralLayoutConfig } = this.options;
    const {
      ...
      for (const image of images) {
        ...
        while (([dx, dy] = spiral((t += dt)))) {
          ...
          if (fit(fillingImage, fixedFillingImages) && fit(fillingImage, fixedImages)) {
            ...
            if (!intersect) {
              fillingImage._frequency = image._frequency++;
              fillingImage.visible = true;
              fillingImage.opacity = fillingOpacity;
              fillingImage[key] = `${fillingImage[key]}_${fillingImage._frequency}`;
    ...
    visualizeSegmentation(segmentationOutput.segmentation.labels);
